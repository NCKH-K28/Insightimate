    ...
    <div className='space-y-6'>
      ...
      <Tabs defaultValue='overview' className='space-y-4'>
        <TabsList className='grid w-full grid-cols-5 lg:w-auto lg:inline-flex'>
          ...
          <TabsTrigger value='outcome' className='text-xs sm:text-sm'>
            Outcome
          </TabsTrigger>
        </TabsList>
            ...
            <Card>
              ...
              <CardContent>
                <div className='grid grid-cols-4 gap-2 mb-4'>
                  <MetricCard
                    label='Created'
                    value={quality.totalBugsCreated}
                    className='p-2 bg-red-50'
                  />
                  <MetricCard
                    label='Resolved'
                    value={quality.totalBugsResolved}
                    className='p-2 bg-green-50'
                  />
                  <MetricCard
                    label='Net'
                    value={
                      quality.netBugChange >= 0 ? `+${quality.netBugChange}` : quality.netBugChange
                    }
                    className={`p-2 ${quality.netBugChange > 0 ? 'bg-red-50' : 'bg-green-50'}`}
                  />
                  {quality.bugWorkRatio !== undefined && (
                    <MetricCard
                      label='Bug Ratio'
                      value={`${Math.round(quality.bugWorkRatio * 100)}%`}
                      className='p-2'
                    />
                  )}
                </div>
                <div className='h-48'>
                  <ResponsiveContainer width='100%' height='100%'>
                    <LineChart data={quality.bugTrend}>
                      <CartesianGrid strokeDasharray='3 3' className='opacity-50' />
                      <XAxis dataKey='dayLabel' tick={{ fontSize: 10 }} tickLine={false} />
                      <YAxis tick={{ fontSize: 10 }} tickLine={false} />
                      <RechartsTooltip content={<CustomTooltip />} />
            ...
            <Card>
              <CardHeader className='pb-2'>
                <div className='flex items-center justify-between'>
                  <SectionHeader
                    ...
                    title='Top Bugs'
                    description='By severity & age'
                  />
                  {onOpenBugsDetails && (
                    <Button variant='ghost' size='sm' onClick={onOpenBugsDetails}>
                      <ExternalLink className='h-4 w-4 mr-1' />
                      View All
                    </Button>
                  )}
              ...
              <CardContent>
                <ScrollArea className='h-64'>
                  {quality.bugSummary.length > 0 ? (
                    <div className='space-y-2'>
                      {quality.bugSummary.slice(0, 8).map((bug) => (
                        <div
                          key={bug.id}
                        ...
                        >
                          <div className='flex-1 min-w-0'>
                            ...
                            <p className='text-sm truncate mt-0.5'>{bug.title}</p>
                          </div>
                    ...
                    <div className='flex flex-col items-center justify-center py-8 text-muted-foreground'>
                      ...
                      <p className='text-sm'>No bugs in this sprint! </p>
                    </div>
                  )}
                </ScrollArea>
              </CardContent>
        ...
        <TabsContent value='outcome' className='space-y-4'>
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
            ...
            <Card>
              <CardHeader className='pb-2'>
                <div className='flex items-center justify-between'>
                  <SectionHeader icon={<Target className='h-5 w-5' />} title='Sprint Goal' />
                  <GoalOutcomeBadge outcome={outcome.goalOutcome} />
                </div>
              </CardHeader>
              <CardContent>
                {outcome.goalDescription && (
                  <div className='p-3 bg-muted/30 rounded-lg mb-4'>
                    <p className='text-sm font-medium'>{outcome.goalDescription}</p>
                  </div>
                )}
                {outcome.narrativeSummary && (
                  <div className='p-3 bg-blue-50 rounded-lg mb-4'>
                    <p className='text-sm text-blue-800'>{outcome.narrativeSummary}</p>
                  </div>
                )}
                <Separator className='my-4' />
                {/* Key Learnings */}
                {outcome.keyLearnings && outcome.keyLearnings.length > 0 && (
                  <div className='mb-4'>
                    ...
                    <ul className='space-y-1.5'>
                      {outcome.keyLearnings.map((learning, idx) => (
                        <li
                          key={idx}
                          className='text-sm flex items-start gap-2 p-2 bg-yellow-50/50 rounded-md'
                        >
                  ...
                  <div>
                    ...
                    <ul className='space-y-1.5'>
                      {outcome.actionItems.map((item, idx) => (
                        <li
                          key={idx}
                          className='text-sm flex items-start gap-2 p-2 bg-blue-50/50 rounded-md'
                        >
            ...
            <Card>
              <CardHeader className='pb-2'>
                <SectionHeader
                  ...
                  title='Carryover'
                  description='Work moving to next sprint'
                />
              </CardHeader>
              <CardContent>
                <div className='grid grid-cols-2 gap-3 mb-4'>
                  <MetricCard
                    label='Issues'
                    value={outcome.carryoverIssueCount ?? outcome.carryoverIssues.length}
                    icon={<Layers className='h-4 w-4' />}
                  />
                  <MetricCard
                    label='Story Points'
                    value={outcome.carryoverPoints ?? 0}
                    icon={<Target className='h-4 w-4' />}
                  />
                </div>
                <Separator className='my-3' />
                <ScrollArea className='h-52'>
                  {outcome.carryoverIssues.length > 0 ? (
                    <div className='space-y-2'>
                      {outcome.carryoverIssues.map((issue) => (
                        <div
                          key={issue.id}
                          className='flex items-center justify-between p-2 bg-muted/30 rounded-md'
                        >
                          ...
                          <div className='flex items-center gap-2 flex-shrink-0 ml-2'>
                            ...
                            <ReasonBadge reason={issue.reasonCategory} />
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className='flex flex-col items-center justify-center py-8 text-muted-foreground'>
                      <CheckCircle className='h-8 w-8 mb-2 text-green-500' />
                      <p className='text-sm'>No carryover – great job!</p>
                    </div>
                  )}
                </ScrollArea>
              </CardContent>
