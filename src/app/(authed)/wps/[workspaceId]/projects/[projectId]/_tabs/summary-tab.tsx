...
export const SummaryTab: React.FC<{params : {projectId: string; workspaceId: string; boardId: string}}> = ({params}) => {
  ...
  const priorityChartDataFromApi = (data?.priorityData && Array.isArray(data.priorityData) && data.priorityData.length > 0)
    ? data.priorityData.map((p: any) => ({ priority: p.label, issues: p.value }))
    : priorityChartData;
  const maxPriorityValue = Math.max(...priorityChartDataFromApi.map((d: any) => Number(d.issues) || 0), 1)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
      ...
      <aside className="lg:col-span-1 flex flex-col gap-6">
        <ProjectOverview workspaceName={workspaceName} projectLead={projectLead} startDate={startDate} version={version} />
