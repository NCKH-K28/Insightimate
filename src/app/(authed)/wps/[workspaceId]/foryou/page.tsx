...
function ProjectCard({
  ...
  return (
    <Card className="relative overflow-hidden group border-muted/60 hover:border-muted transition-colors">
      {/* Color stripe */}
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h3 className="text-base font-medium leading-tight truncate">{name}</h3>
    ...
    <div className="flex items-start gap-3 rounded-xl border p-3 hover:bg-accent/30">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {checked && <Badge variant="outline" className="h-5">Done</Badge>}
        </div>
    ...
    <div className="mx-auto w-full px-6 py-6 md:py-8">
      ...
      <section className="mt-6">
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {recentSpaces.map((p) => (
            <ProjectCard key={p.id} name={p.name} type={p.type} color={p.color} openItems={p.openItems} doneItems={p.doneItems} boards={p.boards} />
          ))}
        </div>
      </section>
        ...
        <Tabs defaultValue="viewed" className="w-full">
          <TabsList className="grid w-full grid-cols-5">
            ...
            <TabsTrigger value="assigned" className="gap-1 text-xs">
              ...
              <Badge variant="secondary" className="ml-2 h-5">{assignedCount}</Badge>
            </TabsTrigger>
          ...
          <TabsContent value="viewed" className="mt-4">
            ...
            <div className="mt-2 space-y-2">
              {today.map((row) => (
                <TodayRow key={row.id} title={row.title} meta={row.meta} checked={row.checked} Icon={row.icon} />
              ))}
            </div>
