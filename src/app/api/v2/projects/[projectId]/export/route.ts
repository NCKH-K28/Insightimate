...
const exportProject = async (projectId: string): Promise<ProjectExport> => {
  const { project, teams } = await prisma.$transaction(async (tx) => {
    ...
    const teams = teamIds.length
      ? await tx.team.findMany({ where: { id: { in: teamIds } }, include: { members: true } })
      : [];
...
// GET /api/v2/projects/[projectId]/export
export const GET = compose<{ projectId: string }>(async (req) => {
  const { params } = req;

  const result = await exportProject(params.projectId);
  const mode = 'download';
