...
export const Comment = ({
  ...
  const [isCommentReactionOpen, setIsCommentReactionOpen] = React.useState(false);
  ...
  return (
    <div
      ...
      onMouseLeave={() => !hasReactions && !isCommentReactionOpen && setShowReactionButton(false)}
    >
      <Avatar>
        <AvatarImage src={getAvatar(comment.data.authorName)} alt={comment.data.authorName} />
        <AvatarFallback>{comment.data.authorName?.toUpperCase()[0]}</AvatarFallback>
      </Avatar>
          ...
          <div className='tiptap-comment-details'>
            <div className='tiptap-comment-information'>
              <div className='tiptap-comment-meta'>
                <div className='tiptap-comment-author'>{comment.data.authorName}</div>
                <div className='tiptap-comment-date'>
                  ...
                  {comment.data.editedAt && (
                    <Tooltip>
                      <TooltipTrigger>
                        <div className='tiptap-comment-date-edited'>(edited)</div>
                      </TooltipTrigger>
                      <TooltipContent>{dayjs(comment.data.editedAt).fromNow()}</TooltipContent>
                    </Tooltip>
                  )}
                </div>
              </div>
              ...
              {isCommentOwner && (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button data-size='small' data-style='ghost' tooltip='Comment options'>
                      ...
                      <CardBody>
                        <ButtonGroup>
                          <DropdownMenuItem asChild>
                            <Button data-style='ghost' onClick={() => setIsEditing(true)}>
                              <PencilIcon className='tiptap-button-icon' />
                              <span className='tiptap-button-text'>Edit comment</span>
                            </Button>
                          </DropdownMenuItem>
                          ...
                          {!hideDelete && (
                            <DropdownMenuItem asChild>
                              <Button data-style='ghost' onClick={handleDelete}>
                                ...
                                <span className='tiptap-button-text'>Delete comment</span>
                              </Button>
                            </DropdownMenuItem>
                          )}
                        </ButtonGroup>
