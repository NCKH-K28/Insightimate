...
import { ChevronDown, X, CheckIcon, LoaderIcon } from 'lucide-react';
...
export const MultiAsyncSelect = React.forwardRef<MultiAsyncSelectRef, Props>(
  (
    ...
    useUpdateEffect(() => {
      ...
      if (async) {
        if (!isInit.current) {
          ...
          optionsRef.current = { ...temp, ...temp2 };
          setReserveOptions({ ...temp, ...temp2 });
        }
      }
    ...
    useUpdateEffect(() => {
      if (searchValue !== undefined) setSearchValueState(searchValue);
    }, [searchValue]);
      ...
      >
        ...
        >
          <Command shouldFilter={!async}>
            ...
            <CommandList>
              {async && error && (
                <div className='p-4 text-destructive text-center'>{error.message}</div>
              )}
