...
export type GanttStatus = IssueField;
export type GanttFeature = IssueType;
type GanttFeatureWithDates = GanttFeature & { startAt: Date; endAt: Date };
...
export type GanttContextProps = {
  ...
  scrollToFeature?: (feature: GanttFeatureWithDates) => void;
};
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  const duration = useMemo(() => {
    if (!feature.startAt || !feature.endAt) return null;
    const tempEndAt =
  ...
  const handleClick: MouseEventHandler<HTMLDivElement> = (event) => {
    const { startAt, endAt } = feature;
    if (!startAt || !endAt) return;
    if (event.target === event.currentTarget) {
      // Scroll to the feature in the timeline
      gantt.scrollToFeature?.({ ...feature, startAt, endAt });
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ backgroundColor: feature.status?.color ?? undefined }}
      />
      <p className='pointer-events-none flex-1 truncate text-left font-medium'>{feature.summary}</p>
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  const [startAt, setStartAt] = useState(feature.startAt ?? null);
  const [endAt, setEndAt] = useState(feature.endAt ?? null);
  // Memoize expensive calculations
  const width = useMemo(() => {
    if (!startAt) return 0;
    return getWidth(startAt, endAt, gantt);
  }, [startAt, endAt, gantt]);
  const offset = useMemo(
    () => (startAt ? getOffset(startAt, timelineStartDate, gantt) : 0),
  ...
  const handleItemDragMove = useCallback(() => {
    if (!previousStartAt) return;
    if (previousMouseX === 0) return;
    if (mousePosition.x === previousMouseX) return;
          ...
          <>
            ...
            >
              <GanttFeatureItemCard id={feature.id}>
                {children ?? <p className='flex-1 truncate text-xs'>{feature.summary}</p>}
...
export type GanttFeatureRowProps = {
  features: GanttFeatureWithDates[];
  onMove?: (id: string, startAt: Date, endAt: Date | null) => void;
  children?: (feature: GanttFeatureWithDates) => ReactNode;
  className?: string;
};
