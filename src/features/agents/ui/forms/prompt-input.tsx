...
import { AllSourcesDropdown } from './all-sources-dropdown';
import { SelectionBar } from '../components/ai-chat-input/selection-bar';

type InputContext = { id: string; name: string; type: string; iconURL?: string };

export type PromptInputProps = {
  onSubmit?: (value: string) => void;
  submitDisabled?: boolean;
  defaultContexts?: InputContext[];
  onContextsChange?: (contexts: InputContext[]) => void;
};
    ...
    <form onSubmit={handleSubmit}>
      <div className='bg-background border border-border rounded-2xl overflow-hidden'>
        ...
        <div className='px-1 py-1'>
          <SelectionBar
            defaultSelecteds={defaultContexts?.map((ctx) => ({
              label: ctx.name,
              value: ctx.id,
              iconURL: ctx.iconURL,
              type: ctx.type,
