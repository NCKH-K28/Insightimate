...
export const useThreadPositionRect = ({
  ...
  React.useEffect(() => {
    ...
    editor.view.root.addEventListener(
      "scroll",
    ...
    return () => {
      observer.disconnect()
      editor.view.root.removeEventListener("scroll", () =>
        requestAnimationFrame(updateRect)
      )
    }
  ...
  return React.useMemo(() => {
    ...
    editor.state.doc.descendants((node, pos) => {
      ...
      const isBlockThread =
        node.type.name === "blockThread" &&
        node.attrs["data-thread-id"] === activeThread

      const isInlineThread = node.marks.some(
        (mark) =>
          mark.type.name === "inlineThread" &&
          mark.attrs["data-thread-id"] === activeThread
      )
    ...
    const getBubbleRect = () => {
      return (
        bubbleRect ||
        threadBubbles[activeThread]?.[0]?.getBoundingClientRect() ||
        null
      )
    }
